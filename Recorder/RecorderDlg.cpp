...
#pragma endregion E_CHG_SpyState
		...
		case E_CHG_SpyHangupInfo:{
#pragma region E_CHG_SpyHangupInfo
			int nCic = wParam;
			if (nCic >= 0 && nCic <= this->nMaxCh)
			{
				if (ChMap[nCic].nState == STATE_RECORDING)
...
#pragma endregion E_CHG_SpyHangupInfo
		}
		break;
...
#pragma region E_CHG_PcmLinkStatus
			LOG4CPLUS_DEBUG(log, "Pcm:" << wParam << " E_CHG_PcmLinkStatus:" << lParam);
#pragma endregion E_CHG_PcmLinkStatus
		}
		case E_PROC_RecordEnd:{
#pragma region E_PROC_RecordEnd
			INT32 nCic = MySpyChToCic(wParam);
			LOG4CPLUS_TRACE(log, "Ch:" << nCic << " E_PROC_RecordEnd");
