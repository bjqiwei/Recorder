...
{
	m_pDataLink = NULL;
	m_pDataLink.CreateInstance(__uuidof(MSDASC::DataLinks));
	try
	{
		...
		CString strReturn = COLE2CT(conn->GetConnectionString().copy());
...
CString COLEDBDataLink::Edit(LPCSTR lpstrConnectionString, HWND hWnd)
{
	BOOL bRet;
	m_pDataLink = NULL;
